-- Fix the stats function with correct JOINs
DROP FUNCTION IF EXISTS public.stats_statuts_eleves(date, text);
...
DECLARE
  ...
  avancement numeric;
BEGIN
  -- Parse année scolaire (ex: "2025_2026")
  ...
  -- Calculer l'avancement de l'année scolaire
  ...
  RETURN QUERY
  WITH dossiers_filtres AS (
    SELECT 
      d.id, 
      d.tarif_scolarite, 
      COALESCE(d.impaye_anterieur, 0) AS impaye
  ...
  reg AS (
    SELECT 
      r.dossier_id, 
      COALESCE(SUM(r.montant), 0) AS total_verse, 
      MAX(r.date_reglement) AS dernier_reglement
    FROM reglements r
    INNER JOIN dossiers_filtres df ON df.id = r.dossier_id
  ...
  ech_echues AS (
    SELECT 
      e.dossier_id, 
      COALESCE(SUM(e.montant), 0) AS du_echu
    FROM echeances e
    INNER JOIN dossiers_filtres df ON df.id = e.dossier_id
  ...
  SELECT 
    statut_calcule AS name, 
    COUNT(*)::int AS value
  FROM statuts
  WHERE statut_calcule <> 'Non renseigné'
  GROUP BY statut_calcule;
END;
$$;